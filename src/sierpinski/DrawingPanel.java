package sierpinski; /**
 The sierpinski.DrawingPanel class provides a simple interface for drawing persistent
 images using a Graphics object.  An internal BufferedImage object is used
 to keep track of what has been drawn.  A client of the class simply
 constructs a sierpinski.DrawingPanel of a particular size and then draws on it with
 ...
 This version of sierpinski.DrawingPanel also saves animated GIFs, though this is kind
...
public final class DrawingPanel extends FileFilter
    ...
    private static void checkAnimationSettings() {
        try {
            ...
            if (settingsFile.exists()) {
                ...
                System.out.println("***");
                System.out.println("*** sierpinski.DrawingPanel saving animated GIF: " +
    ...
    */
    ...
    private long createTime;               // time at which sierpinski.DrawingPanel was constructed
    ...
    public DrawingPanel(int width, int height) {
        ...
        synchronized (getClass()) {
            instances++;
            instanceNumber = instances;  // each sierpinski.DrawingPanel stores its own int number
        ...
        if (isGraphical() && frame.isVisible()) {
            ...
            if (millis > 0) {
                try {
                    Thread.sleep(millis);
                    //panel.repaint();
                    imagePanel.paintComponent(panel.getGraphics());
                    toFront(frame);
                } catch (Exception e) {}
            }
        }
    ...
    // called when sierpinski.DrawingPanel closes, to potentially exit the program
    ...
    // compares current sierpinski.DrawingPanel image to an image file on disk
    ...
    // compares current sierpinski.DrawingPanel image to an image file on the web
...
*/
    ...
    private void saveAsAnimated() {
        ...
        if (filename != null) {
            try {
                ...
                JOptionPane.showMessageDialog(frame,
                        "Due to constraints about how sierpinski.DrawingPanel works, you'll need to\n" +
                                "re-run your program.  When you run it the next time, sierpinski.DrawingPanel will \n" +
    ...
    // saves sierpinski.DrawingPanel image to a temporary file and returns file's name
    ...
    // initializes sierpinski.DrawingPanel's menu bar items
